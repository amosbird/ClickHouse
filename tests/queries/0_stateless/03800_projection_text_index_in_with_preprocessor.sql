...
-- Adapted from 04043_text_index_in_with_preprocessor.sql for projection text indexes.
...
CREATE TABLE tab_in_preprocess (
    id UInt64,
    text String,
    PROJECTION idx INDEX text TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(text))
) ENGINE = MergeTree()
ORDER BY id
