...
-- Adapted from 04039_text_index_direct_read_select_alias.sql for projection text indexes.
...
(
    `id` UInt32,
    `title` String,
    PROJECTION title_idx INDEX title TYPE text(tokenizer = 'splitByNonAlpha')
)
ENGINE = MergeTree
ORDER BY id
