-- Tests for projection text index on Nullable columns and arrays with Nullable elements.
...
-- Adapted from 02346_text_index_nullable.sql
...
(
    id  UInt32,
    arr Array(Nullable(String)),
    PROJECTION idx INDEX arr TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(arr))
)
ENGINE = MergeTree
ORDER BY id;
...
(
    ...
    PROJECTION idx INDEX mapValues(m) TYPE text(tokenizer = 'splitByNonAlpha')
)
ENGINE = MergeTree
ORDER BY id;
