...
(
    ...
    PROJECTION map_keys INDEX mapKeys(map) TYPE text(tokenizer = splitByNonAlpha),
    PROJECTION map_values INDEX mapValues(map) TYPE text(tokenizer = splitByNonAlpha),
    PROJECTION map_keys_fixed INDEX mapKeys(map_fixed) TYPE text(tokenizer = splitByNonAlpha),
    PROJECTION map_values_fixed INDEX mapValues(map_fixed) TYPE text(tokenizer = splitByNonAlpha)
)
ENGINE = MergeTree
ORDER BY (id);
