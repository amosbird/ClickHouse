...
public:
    ...
    /** If the data is not sorted, but we have previously calculated the permutation, that will sort it.
      ...
      */
    void writeWithPermutation(const Block & block, const IColumn::Permutation * permutation, Block * permuted_columns_cache);
...
private:
    /** If `permutation` is given, it rearranges the values in the columns when writing.
      ...
      */
    void writeImpl(const Block & block, const IColumn::Permutation * permutation, Block * permuted_columns_cache);
