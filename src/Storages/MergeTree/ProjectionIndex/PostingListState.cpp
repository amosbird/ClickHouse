...
static Array convertASTArgumentsToParams(const ASTPtr & arguments)
{
    ...
    for (const auto & child : arguments->children)
    {
        ...
        Tuple tuple;
        tuple.emplace_back(key->name());

        /// For preprocessor arguments the value is an expression (ASTFunction), not a literal.
        /// Store its string representation, matching the old serialization behavior.
        const auto * literal = func->arguments->children[1]->as<ASTLiteral>();
        if (literal)
            tuple.emplace_back(literal->value);
        else
            tuple.emplace_back(func->arguments->children[1]->getColumnName());

        params.emplace_back(std::move(tuple));
    }
    return params;
}
...
DataTypePtr createPostingListTypeFromPartMetadata(const ASTPtr & parsed_fields)
{
    ...
    for (const auto & param : params)
    {
        ...
        const auto & key = tuple[0].safeGet<String>();
        if (key == "dictionary_block_size")
            posting_list_params.dictionary_block_size = tuple[1].safeGet<UInt64>();
        else if (key == "dictionary_block_frontcoding_compression")
            posting_list_params.dictionary_block_frontcoding_compression = tuple[1].safeGet<UInt64>();
        else if (key == "posting_list_block_size")
            posting_list_params.posting_list_block_size = tuple[1].safeGet<UInt64>();
        else if (key == "preprocessor")
            posting_list_params.preprocessor = make_intrusive<ASTLiteral>(tuple[1]);
        /// "tokenizer" and "posting_list_codec" are not needed in PostingListParams;
        /// they are handled separately by MergeTreeIndexText. Skip silently.
    }
