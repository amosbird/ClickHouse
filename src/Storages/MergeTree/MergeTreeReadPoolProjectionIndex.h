...
struct ProjectionIndexBitmap
{
    ...
    /// Fills a ColumnUInt8 (as `UInt8 *`) in the range [start, end) with 1s at positions present in the bitmap.
    ///
    /// For example, if the bitmap contains values {101, 103} and `start = 100`, `end = 105`, the resulting buffer (of
    /// size `end - start`) will be: [0, 1, 0, 1, 0].
    ///
    /// Returns:
    /// - true if at least one value in the bitmap falls within the specified range.
    /// - false if there are no matching values.
    ///
    /// The caller must ensure the `pos` buffer is zero-initialized before calling this function.
    template <typename Offset>
    bool fillBits(UInt8 * pos, size_t start, size_t end) const
    {
        static_assert(
            std::is_same_v<Offset, UInt32> || std::is_same_v<Offset, UInt64>,
        ...
        if constexpr (std::is_same_v<Offset, UInt32>)
        {
            ...
            if (!roaring_uint32_iterator_move_equalorlarger(&it, start))
                return false;

            bool has_value = false;
            while (it.current_value < end)
            {
                has_value = true;
                pos[it.current_value - start] = 1;
        ...
        else
        {
            ...
            if (!roaring::api::roaring64_iterator_move_equalorlarger(it, start))
            {
...
public:
    ...
    /// Keyed by raw pointer of IMergeTreeDataPart to ensure identity.
