            ...
            while (true)
            {
                ...
                if (chunk.chunk)
                {
                    if (chunk.chunk.getNumRows() > 0)
                    {
                        ...
                        const auto & offsets = assert_cast<const ColumnUInt64 &>(*chunk.chunk.getColumns()[0]);
...
void MergeTreeSelectProcessor::initializeReadersChain()
{
    ...
    if (projection_index_build_context)
    {
        ...
        {
            projection_index_builder = [this, &projection_index_all_ranges = it->second, &remaining_marks]()
            {
