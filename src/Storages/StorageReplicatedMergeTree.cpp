...
void StorageReplicatedMergeTree::checkBrokenDisks()
{
    ...
    for (auto disk_it = disks.rbegin(); disk_it != disks.rend(); ++disk_it)
    {
        ...
        if (disk_ptr->isBroken())
        {
            ...
            LOG_INFO(log, "Scanning parts to recover on broken disk {}", disk_ptr->getName() + "@" + disk_ptr->getPath());
        ...
        {
            {
                ...
                if (last_broken_disks.erase(disk_ptr->getName()) > 0)
                    LOG_INFO(
                        log, "Disk {} is recovered. Exclude it from last_broken_disks", disk_ptr->getName() + "@" + disk_ptr->getPath());
            }
        }
    }
}
