...
struct ABHashTableHash
{
    size_t ALWAYS_INLINE operator()(StringRef key) const
    {
        ...
        if ((reinterpret_cast<uintptr_t>(key.data) >> 63) == 1) [[unlikely]]
            return StringRefHash()({reinterpret_cast<const char *>(reinterpret_cast<uintptr_t>(key.data) & ~(1ULL << 63)), key.size});
...
struct ABHashMapCell
{
    ...
    {
        if (this->value.first.data == nullptr)
            return {};

        if ((reinterpret_cast<uintptr_t>(this->value.first.data) >> 63) == 1) [[unlikely]]
        {
            return {
                reinterpret_cast<const char *>(reinterpret_cast<uintptr_t>(this->value.first.data) & ~(1ULL << 63)),
                this->value.first.size};
        }

        size_t inlined_size = reinterpret_cast<uintptr_t>(this->value.first.data) >> 59;
        if (inlined_size > 0)
    ...
    bool keyEquals(const Key & key_) const
    {
        ...
        if ((reinterpret_cast<uintptr_t>(value.first.data) >> 63) == 1) [[unlikely]]
        {
    ...
    bool keyEquals(const Key & key_, size_t /* hash_ */) const { return keyEquals(key_); }
    bool keyEquals(const Key & key_, size_t /* hash_ */, const State & /*state*/) const { return keyEquals(key_); }
